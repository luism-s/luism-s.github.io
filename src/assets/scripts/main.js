...
function renderWorks() {
  ...
  var template, rendered;
    ...
    if ( data ) {
      ...
      $('#works').html( rendered );
      $('[work_id=0] .button__navigation.-left').addClass( '-inactive' );
      $('[work_id=' + ( data.works.length - 1 ) + '] .button__navigation.-right').addClass( '-inactive' );
    }
  } );
}
...
function hideWork( work_id ) {
  'use strict';
  if ( work_id >= 0 ) {
    $('[work_id=' + work_id + '] .js-work-showcase').removeClass( '-shown' );
  } else {
    $('.js-work-showcase').removeClass( '-shown' );
  }
  $('.dimmed_background').removeClass( '-active' );
}

function showWork( work_id ) {
  var showcase = '[work_id=' + work_id + '] .js-work-showcase';
  if ( work_id >= 0 && !$( showcase ).hasClass( "-shown" ) ) {
    $(showcase).addClass( '-shown' );
    $('.dimmed_background').addClass( '-active' );
...
/*   renderWorks();
  ...
  $( document ).on( 'click', '.js-switch-work', function () {
    ...
    if ( $( this ).hasClass( '-previous' ) && work_id > 0 ) {
      next_work_id--;
      workTracking( next_work_id, 'swich/backwards' );
    } else if ( $( this ).hasClass( '-next' ) && work_id < ( number_of_works - 1 ) ) {
      next_work_id++;
